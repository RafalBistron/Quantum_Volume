...
def get_means_and_deviations(distributions_tab):
...
def simple_exponent(x,a,b,c):
    return a*np.exp(-b*x) +c

def two_exponents(x,a,b,c):
...
def show_heavy_outputs(heavy_outputs_tabs, T_tab, N):
    ...
    for i, hu_tab in enumerate(heavy_outputs_tabs):
        means, sigmas = get_means_and_deviations(hu_tab)
        ...
        if i < 3:
            func = simple_exponent
        else:
            func = two_exponents
            
        fit_coef, fit_cov = curve_fit(func, T_tab, means, sigma = sigmas, p0 = [2,1,0.5],maxfev=5000,bounds = ([0,0,0],[3,np.inf,1])) 

        ax.plot(T_tab, func(T_tab,*fit_coef),c = color_tab[i])
        ax.plot(T_tab,0*T_tab+2/3,c = 'black',linestyle = "-.")
        ax.fill_between(T_tab, func(T_tab,*(fit_coef + np.diag(fit_cov))), func(T_tab,*(fit_coef - np.diag(fit_cov))), alpha=0.1,color = color_tab[i])            
